...
ggplot(grid, aes(x = b, y = e, fill = f)) + geom_raster() + scale_fill_viridis_c(name = "Density", limit=c(0,2)) + labs( x = "Bet size (fraction of pot)", y = "Equity", title = expression(paste("Joint density f(e,b) = 1 + ", lambda, "(2e-1)(2b-1)")) ) + theme_minimal()
```
------------------------------------------------------------------------
...
### Worked Example: \(\mathbb{P}(E>0.75 \mid B=0.75)\) from Our Joint Density

Let \(E\in[0,1]\) denote **equity** and \(B\in[0,1]\) denote **bet size in pot units**.  
In our model, the **joint density** of \((E,B)\) on the unit square is
...
(For this to be a valid density everywhere on \([0,1]^2\), we need \(|\lambda|\le 1\), since \((2e-1)(2b-1)\in[-1,1]\).)
...
#### Step 1: Compute the marginal density of \(B\)
...
So the marginal is especially simple:

$$
\boxed{f_B(b)=1 \quad \text{for } 0\le b\le 1.}
$$

That is, \(B\) is uniformly distributed on \([0,1]\) under this joint model.

---

#### Step 2: Write down the conditional density of \(E\) given \(B=b\)

For a continuous pair \((E,B)\), the conditional density is

$$
f_{E\mid B}(e\mid b) \;=\; \frac{f_{E,B}(e,b)}{f_B(b)}.
$$

Since \(f_B(b)=1\), we get
...
Now plug in \(b=0.75\). Since \(2b-1 = 2(0.75)-1 = 0.5\),
...
so the integral equals \(0 - (-0.09375)=0.09375 = \frac{3}{32}\). Therefore,
...
- If \(\lambda=0\), then \(f_{E,B}(e,b)=1\) (independence/uniform on the square), and
  $$
  \mathbb{P}(E>0.75 \mid B=0.75)=\frac{1}{4}.
  $$
- If \(\lambda>0\), then higher bets are associated with higher equity, so conditioning on \(B=0.75\) increases the probability above \(1/4\).
- If \(\lambda<0\), it decreases the probability below \(1/4\).

(Recall \(|\lambda|\le 1\) keeps the density nonnegative everywhere.)
