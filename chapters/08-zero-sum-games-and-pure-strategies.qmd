...
1. **Constant–Sum Games**
   ...
   Consider a two-player normal–form game between **Hero (H)** and
   **Villain (V)**.
   ...
   denote the set of strategies available to Hero and

   \[
   V
   \]

   denote the set of strategies available to Villain.

   The payoff functions are

   \[
   u_H : H \times V \to \mathbb{R}, 
   \qquad
   u_V : H \times V \to \mathbb{R}.
   \]

   Suppose a second game has payoff functions $u_H',u_V'$.

   We say the two games are **strategically equivalent** if there exist
   constants $a_H,a_V>0$ and functions
   ...
   such that for every pair of strategies $(h,v) \in H \times V$,

   \[
   u_H'(h,v) = a_H\,u_H(h,v) + c_H(v),
   \]

   \[
   u_V'(h,v) = a_V\,u_V(h,v) + c_V(h).
   \]

   Intuitively:

   - Hero’s payoffs may be multiplied by a positive constant and have a
     constant added to each **column** (depending only on Villain’s
     strategy).
   - Villain’s payoffs may be multiplied by a positive constant and have
     a constant added to each **row** (depending only on Hero’s
     strategy).



**(a) Preservation of strategy comparisons**

1. Fix a Villain strategy $v \in V$ and two Hero strategies
   $h,h' \in H$. Show that

   \[
   u_H'(h,v)-u_H'(h',v)
   =
   a_H\big(u_H(h,v)-u_H(h',v)\big).
   \]

   Conclude that Hero ranks $h$ and $h'$ the same way in both games.

2. **Example.**

   Suppose Hero’s payoff matrix is
   ...
   Construct a strategically equivalent matrix $A'$ by

   - multiplying all entries by $2$,
   - adding $+3$ to the first column,
   - adding $-1$ to the second column.

   Compute $A'$ and verify that Hero’s preferred row in each column
   is unchanged.



**(b) Constant–sum games**

A two-player game is **constant–sum** if there exists a constant
$c$ such that
...
for every $(h,v)\in H \times V$.

Prove that every constant–sum two-player game is strategically
...
Consider the constant–sum game with payoff matrices
...
1. Verify that this game is constant–sum and determine the constant.

2. Construct payoff matrices $(A^{(0)},B^{(0)})$ for a strategically
equivalent **zero–sum game**.
2. **Maximin Inequality (Pure Strategies)**
  
  (a) Prove that for every real matrix $A$,
$$
  \max_i \min_j A_{ij}
\;\le\;
\min_j \max_i A_{ij}.
...
$$
  ...
  (a) Identify all strictly or weakly dominated strategies (rows and/or columns).  
(b) Reduce the matrix by iterated elimination of dominated strategies.  
(c) Determine whether a saddle point exists in the reduced game.  
(d) If a saddle point exists, compute the value and give optimal pure strategies.

4. **Crossed Saddle Points**
  
  Let $A$ be the payoff matrix of a finite two–player zero–sum game
(payoff to the row player).

Suppose that $(i_a, j_a)$ and $(i_b, j_b)$ are saddle points.
Denote the saddle value by
$$
  v_a = A_{i_a j_a}, v_b=A_{i_b j_b}.
$$
...
$$
  ...
  (b) Prove that both $(i_a, j_b)$ and $(i_b, j_a)$ are themselves saddle points.

Your proof must rely only on the defining inequalities of a saddle point and should
explicitly indicate where each inequality is used.

5. **Rectangular Structure of Pure Equilibria**
...
(a) Prove that for every $i \in R^*$ and every $j \in C^*$,
$$
  A_{ij} = v.
$$
  
  (b) Conclude that every pair in $R^*\times C^*$ is a saddle point.

(c) In complete sentences, explain why this result implies that whenever
a zero–sum game has more than one pure–strategy equilibrium, the set of equilibria
must form a rectangular block in the payoff matrix.
...
6. **A Poker-Flavored Pure Game with a Saddle Point (New)**
  
  Consider the payoff matrix (to the row player)
$$
  A=
  \begin{pmatrix}
1 & 1 & 1\\
...
$$
  ...
  (a) Compute $\max_i \min_j A_{ij}$ and $\min_j \max_i A_{ij}$.  
(b) Determine whether a saddle point exists. If so, identify *all* saddle points.  
(c) Give a short interpretation: which row is “safe,” and why?
  
  7. **Outcome vs. Strategy (New, Conceptual)**
  
  A student claims: “If the row player loses a hand, then their strategy was bad.”

(a) Give a concrete counterexample using a $2\times 2$ zero–sum game in which
an optimal pure strategy can still lose in a single play.

(b) In complete sentences, explain the difference between an optimal strategy
and a favorable realized outcome.
