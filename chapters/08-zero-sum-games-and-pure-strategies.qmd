...
1. **Constant–Sum Games**
   ...
   We say that two two–player games are **strategically equivalent** if
   one can be obtained from the other by adding constants to players’
   payoffs in a way that does not change any player’s best–response
   structure.  

   More precisely: two payoff matrices \(A\) and \(\widetilde A\)
   (row-player payoffs) are strategically equivalent if there exist
   vectors \(r\in\mathbb{R}^m\) and \(c\in\mathbb{R}^n\) such that
   \[
   \widetilde A_{ij} = A_{ij} + r_i + c_j
   \]
   for all \(i,j\).  
   Such transformations shift payoffs but do not change the location of
   saddle points or the set of optimal pure strategies.
